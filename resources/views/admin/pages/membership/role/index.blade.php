...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">فهرست نقش ها</li>
        </ol>
    </nav>
    <form action="{{ route('admin.membership.role.destroy') }}" method="post" id="form">
        @csrf  
        <div class="d-flex justify-content-between bd-highlight">
            <div class="p-2 bd-highlight">
                <a class="dt-button add-new btn btn-primary text-white" href="{{ route('admin.membership.role.create') }}">
                    <span><i class="bx bx-plus me-0"></i> ایجاد </span>
                </a>
            </div>
            <div class="p-2 bd-highlight col-4">
                <select class="select-role-group" id="role_filter" data-column="2" data-allow-clear="true">
                    <option value=""></option>
        ...
        <div class="card mt-3">
            <div class="card-datatable table-responsive">
                <table class="table table-hover border-top dataTable no-footer dtr-column collapsed text-center" id="getRoles">
                    <thead>
                    <tr>
                        <th>نام</th>
                        <th>کد یکتا 🔑</th>
                        ...
                        <th>عملیات</th>
                        <th>
                            <input class="form-check-input" type="checkbox" value="" id="checkAll" data-bs-toggle="tooltip"
                            data-bs-placement="top" data-color="primary" title="انتخاب همه">
                        </th>
                    </tr>
                    </thead>
                </table>
...
@section('page-js')
    ...
    <script>
        $(function() {
            const select2 = $('.select-role-group');
            // Default
            if (select2.length) {
    ...
    <script>
        $(function () {
            var table = $('#getRoles').DataTable({
                ...
                serverSide: true,
                responsive: false,
                ajax: "{{ route('admin.membership.role.index') }}",
                columns: [
